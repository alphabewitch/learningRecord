...
class Solution {
    public boolean wordBreak(String s, List<String> wordDict) {
        ...
        for(int i = 1; i <= len; i++) {
            for(int j = 0; j < i; j++) {
                // dp[j]表示s中j下标前面的子串可以被组成
                // 并且j~(i-1)子串也存在，所以dp[i]=true
...
class Solution {
    public int maxProduct(int[] nums) {
        ...
        dp[0][1] = nums[0];
        for(int i = 1; i < len; i++) {
            // dp[][0]记录最大的数，如果当前数是负数，最大数可能是当前数×最小值
...
### 题目45 [跳跃游戏 II](https://leetcode.cn/problems/jump-game-ii/)
...
### 题目121 [买卖股票的最佳时机](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock/)
...
### 题目763 [划分字母区间](https://leetcode.cn/problems/partition-labels/)
...
### 题目35 [搜索插入位置](https://leetcode.cn/problems/search-insert-position/)

```java
class Solution {
    public int searchInsert(int[] nums, int target) {
        int left = 0, right = nums.length-1;
        while(left <= right) { // 注意这里要有等号
            int mid = left + (right - left) / 2;
...
### 题目74 [搜索二维矩阵](https://leetcode.cn/problems/search-a-2d-matrix/)
...
### 题目153 [寻找旋转排序数组中的最小值](https://leetcode.cn/problems/find-minimum-in-rotated-sorted-array/)

```java
class Solution {
    public int findMin(int[] nums) {
        int left = 0, right = nums.length-1;
        while(left < right) { // 注意等号和l、r的更新条件
            int mid = left + (right - left) / 2;
...
class Solution {
    ...
    private void quickSort(int[] nums, int left, int right) {
        if(left < right) {
            int pivote = pertition(nums, left, right);
            quickSort(nums, left, pivote-1);
            quickSort(nums, pivote+1, right);
        }
    }
    private int pertition(int[] nums, int left, int right) {
        int p = nums[left];
        while(left < right) {
...
### 题目155 [最小栈](https://leetcode.cn/problems/min-stack/)

```java
class MinStack {
    Deque<Integer> stack;
    Deque<Integer> minStack; // 辅助栈来保存历届最小值
    ...
    public void push(int val) {
        ...
        else if(minStack.peek() >= val) minStack.push(val); //注意这个等号不能少，可以重复最小值
    }
    
    public void pop() {
        int pop = stack.pop();
        if(!minStack.isEmpty() && pop == minStack.peek()) minStack.pop();
    }
...
### 题目215 [数组中的第K个最大元素](https://leetcode.cn/problems/kth-largest-element-in-an-array/)
...
class Solution {
    ...
    private void buildMaxHeap(int[] nums, int maxSize) {
        for(int i = maxSize/2; i >= 0; i--) {
            maxHeapify(nums, i, maxSize);
        }
    }
    private void maxHeapify(int[] nums, int i, int maxSize) {
        int l = i * 2 + 1, r = i * 2 + 2;
        int maxIndex = i;
        if(l < maxSize && nums[l] > nums[maxIndex]) maxIndex = l;
        if(r < maxSize && nums[r] > nums[maxIndex]) maxIndex = r;
        if(maxIndex != i) {
            swap(nums, i, maxIndex);
            maxHeapify(nums, maxIndex, maxSize);
        }
    }
...
### 题目347 [前 K 个高频元素](https://leetcode.cn/problems/top-k-frequent-elements/)
...
class Solution {
    ...
    public int[] topKFrequent(int[] nums, int k) {
        ...
        disNums = new int[map.size()];
        int index = 0;
        for(HashMap.Entry<Integer,Integer> entry : map.entrySet()) 
            disNums[index++] = entry.getKey();  
        buildMaxHeap(disNums, disNums.length);
        int[] res = new int[k];
        int size = disNums.length;
        index = 0;
        for(int i = disNums.length-1; i >= disNums.length-k; i--) {
            res[index++] = disNums[0];
            swap(disNums, 0, i);
            size--;
            maxHeapify(disNums, 0, size);
        }
        return res;
    }
    private void buildMaxHeap(int[] nums, int maxSize) {
    ...
    private void maxHeapify(int[] nums, int i, int maxSize) {
        int l = i * 2 + 1, r = i * 2 + 2;
        int maxIndex = i;
        if(l < maxSize && map.get(nums[l]) > map.get(nums[maxIndex])) maxIndex = l;
        if(r < maxSize && map.get(nums[r]) > map.get(nums[maxIndex])) maxIndex = r;
        if(maxIndex != i) {
            swap(nums, i, maxIndex);
            maxHeapify(nums, maxIndex, maxSize);
        }
...
### 题目394 [字符串解码](https://leetcode.cn/problems/decode-string/)
...
### 题目739 [每日温度](https://leetcode.cn/problems/daily-temperatures/)
